...
- 📱 参考 RustDesk 交互的移动端辅助按键栏，提供一次性 `Ctrl`/`Alt`/`Shift`、功能键、导航键、shell 符号和软键盘遮挡优化。
...
- 📱 **移动端辅助按键栏**：参考 RustDesk 的一次性 `Ctrl`/`Alt`/`Shift`，提供常用 `Ctrl` 快捷键、`F1`-`F12`、导航键和手机键盘经常隐藏的 shell 符号。
...
5. 📱 移动端可使用辅助按键栏输入一次性修饰键、功能键、导航键和系统键盘难以输入的符号。
6. 📐 按需折叠连接面板、调整终端宽度或进入全屏模式。
7. 🚪 使用完成后关闭终端并退出登录。
